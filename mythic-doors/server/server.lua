...
function CheckPlayerAuth(source, doorPermissionData)
    if not doorPermissionData or next(doorPermissionData) == nil then
        return true
    end
        ...
        if char then
            ...
            for k, v in ipairs(doorPermissionData) do
				if v.type == 'character' then
                    local a = tostring(stateId)
                    local b = tostring(v.SID)
					if a == b then
						return true
					end
...
exports('AddCharacterAccess', function(doorId, charSid)
    local door, doorIndex = GetDoorById(doorId)
    if door then
        if not door.restricted then
            door.restricted = {}
        end

        local hasAccess = false
        for _, restriction in ipairs(door.restricted) do
            if restriction.type == 'character' and tostring(restriction.SID) == tostring(charSid) then
                hasAccess = true
                break
            end
        end
        ...
        if not hasAccess then
            table.insert(door.restricted, { type = 'character', SID = charSid })
...
exports('RemoveCharacterAccess', function(doorId, charSid)
    local door, doorIndex = GetDoorById(doorId)
    if door and door.restricted then
        local newRestricted = {}
        ...
        for _, restriction in ipairs(door.restricted) do
            if restriction.type == 'character' and tostring(restriction.SID) == tostring(charSid) then
                ...
                table.insert(newRestricted, restriction)
            end
        end

        if wasChanged then
            door.restricted = newRestricted
            TriggerClientEvent('Doors:Client:UpdateDynamicDoor', -1, doorIndex, door)
        end
    end
    return true
...
function GetDoorById(doorId)
    if type(doorId) == 'string' then
        local index = DOORS_IDS[doorId]
        if index and _doorConfig[index] then
            return _doorConfig[index], index
        end
    elseif _doorConfig[doorId] then
        return _doorConfig[doorId], doorId
    end
    for i, door in ipairs(_doorConfig) do
