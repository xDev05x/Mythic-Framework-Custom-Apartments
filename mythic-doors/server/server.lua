...
exports('AddCharacterAccess', function(doorId, characterSID)
    local doorIndex = DOORS_IDS[doorId]
    if not doorIndex then return false end

    local door = _doorConfig[doorIndex]
    if not door then return false end

    if not door.dbId then return false end
    ...
    Database.Game:updateOne({
        ...
        if success then
            ...
            for k, v in ipairs(door.restricted) do
                if v.type == 'character' and v.SID == characterSID then
                    p:resolve(true)
                    return
                end
            end
            table.insert(door.restricted, {
                type = 'character',
                SID = characterSID,
            })
        end
...
exports('RemoveCharacterAccess', function(doorId, characterSID)
    local doorIndex = DOORS_IDS[doorId]
    if not doorIndex then return false end
    ...
    Database.Game:updateOne({
        ...
        if success then
            if door.restricted then
                for k, v in ipairs(door.restricted) do
                    if v.type == 'character' and v.SID == characterSID then
                        table.remove(door.restricted, k)
                        break
                    end
                end
            end
